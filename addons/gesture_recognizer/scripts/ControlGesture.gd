...
@export var maxLengthLine : float = 500.0  # Define la longitud máxima de la línea
...
func _process(delta):
	...
	if onDrawing:
		...
		if !a.is_empty():
			var last_point = a.back()
			var new_point = get_global_mouse_position()
			var new_segment_length = last_point.distance_to(new_point)
			current_length = 0.0

			for i in range(1, a.size()):
				current_length += a[i - 1].distance_to(a[i])

			if (current_length + new_segment_length) <= maxLengthLine:
				line.add_point(new_point)
				if Outline:
					outline.add_point(new_point)
		else:
