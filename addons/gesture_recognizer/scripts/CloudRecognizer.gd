...
signal classified_gesture(gestureName : StringName, scale : float)
...
func classify(candidate : Gest):
	...
	var maxDistance : float = 10000000.0 # Ajusta este valor según la escala de tu aplicación
	var bestScale : float = 1.0 # Inicializar escala por defecto
	...
	var matchPercentage : float = 100.0 * (1.0 - (minDistance / maxDistance))+50
	prints("recognized gesture:", gestureClass, ", match percentage:", matchPercentage)
	classified_gesture.emit(gestureClass, matchPercentage)
	return gestureClass
